...
public class WEAK_KMP_With_Next {
    private static class KMPwN {
        public int M;
        public char[] p;
        public int[] next;
        public KMPwN(String pat){
            char[] d=pat.toCharArray();
            M=pat.length();
           p=pat.toCharArray();
            InitialPrefixFunction(pat);
            for (int a:next){
                System.out.println(a);
            }
        }

        public void InitialPrefixFunction(String s) {
            int[] a = new int[M];
            int k =-1;
            a[0]=0;
            for (int i=1; i<M; i++ ) {
    ...
    public static void main(String[] args) {
        KMPwN akmp=new KMPwN("你有几块钱");
        MyList<Integer> al=akmp.search("他说他有三块钱，可是你有几块钱，我不知道你有几块钱，知道你有几块钱的人都死了，所以你有几块钱");
        al.moveToHead();
        for (int i=0;i<al.daxiao;i++){
            System.out.print(al.point.val+" ");
            al.moveRight();
        }
    }
