...
public class LearningQueue {
    public static void main(String[] args) {
        ...
        char opera=' ';
        int inEle=0;
        MyQueue<Integer> theQueue =new MyQueue<>();
        for (int i=0;i<zhixincishu;i++){
            opera=in.next().charAt(0);
            if (opera=='E'){
                inEle=in.nextInt();
                theQueue.enqueue(inEle);
            }
            if (opera=='A'){
                Integer tt=theQueue.getFront();
                if (tt==null){
                    System.out.println();
                }
                else{
        ...
        theQueue.list.moveToHead();
        for (int i=0;i<theQueue.list.daxiao;i++){
            out.print(theQueue.list.point.val+" ");
            theQueue.list.moveRight();
        }
        out.close();
    }
