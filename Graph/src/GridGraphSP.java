...
public class GridGraphSP {
    ...
    private static class WDEdge implements Comparable<WDEdge> {
            ...
            int to;
            double weight;
            public WDEdge(int from,int to,double weight){
                this.from=from;
                this.to=to;
                this.weight=weight;
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n=in.nextInt();
        int m=in.nextInt();
        MyEWDGraph g=new MyEWDGraph(n*m,2*n*m-(m+n));
        int[][] height=new int[n][m];
        for (int i=0;i<n*m;i++){
            height[i/m][i%m]=in.nextInt();
        }
        int i=0; int j=0; double time;
        for (int a=0;a<n*m;a++){
            i=a/m;
            j=a%m;
            time=1/Math.pow(2,height[0][0]-height[i][j]);
            if (i-1>=0){
                g.addEdge(new WDEdge(a,a-m,time));
            }
            if (i+1<=n-1){
                g.addEdge(new WDEdge(a,a+m,time));
            }
            if (j-1>=0){
                g.addEdge(new WDEdge(a,a-1,time));
            }
            if (j+1<=m-1){
                g.addEdge(new WDEdge(a,a+1,time));
            }
        ...
        out.println(df.format(dij.disTo[n*m-1]));
        out.close();
    }
}
